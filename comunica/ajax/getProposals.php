...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'GET') {
	...
	{		
		$user_events_proposedAr = MultiPort::getUserEvents($userObj);
		var_dump ($user_events_proposedAr); die();
	}
	else
		...
		foreach ($eventElementAr as $eventElement)
		{
			...
			if (isset($eventElement[10]))
			{
				...
				foreach ($recipientAr as $recipient)
				{
					...
					{
						if ($x==0) $recipientFName = $val;
						if ($x==1) $recipientLName = $val;
					}
				}
			}
			...
			for ($j=0; $j<count($datesAr); $j++)
			{
				...
				if ($timestamp >= intval($_GET['start']) && $timestamp <= intval($_GET['end']))
				{
					...
					if ($flag & ADA_VIDEOCHAT_EVENT)     $proposedDateAndTimeAr[$i]['type'] = translateFN('Appuntamento in videochat');
					else if ($flag & ADA_CHAT_EVENT)     $proposedDateAndTimeAr[$i]['type'] = translateFN('Appuntamento in chat');
					else if ($flag & ADA_PHONE_EVENT)    $proposedDateAndTimeAr[$i]['type'] = translateFN('Appuntamento telefonico');
					else if ($flag & ADA_IN_PLACE_EVENT) $proposedDateAndTimeAr[$i]['type'] = translateFN('Appuntamento in presenza');
